...
// Nota: usamos API_BASE_URL vindo do arquivo de configuração do projeto
...
const Trilhacard = ({ id, title, image, difficulty, time, distance, description, details, highlights }) => {
  ...
  return (
    <Card>
      <CardImage style={{ backgroundImage: `url('${imgSrc}')` }}>
        {/* elemento img oculto para captar onError e trocar para placeholder se necessário */}
        <img src={imgSrc} alt="" style={{ display: 'none' }} onError={() => setImgSrc(DEFAULT_SVG)} />
  ...
  <CardDesc>{sanitizedDescription}</CardDesc>
        ...
        <Details $active={showDetails} id={id}>
          <DetailsContent>
            ...
            <FeatureList>
              {sanitizedDetails && sanitizedDetails.map((item, idx) => <li key={idx}>{item}</li>)}
            </FeatureList>
            <h3>Destaques</h3>
            <p>{sanitizedHighlights}</p>
